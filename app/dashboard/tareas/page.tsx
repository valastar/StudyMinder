  ...
  return (
    <div className="max-w-2xl mx-auto animate-fade-up">
      <div className="mb-6">
        ...
        <p className="text-sm text-surface-800/50 mt-1">{pending} tareas pendientes</p>
      </div>
        ...
        {(['all', 'pending', 'done'] as const).map((f) => (
          <button
            ...
            className={`px-3 py-1.5 text-xs rounded-lg font-medium transition-all ${
              filter === f ? 'bg-accent text-white' : 'bg-white text-surface-800/50 border border-surface-100 hover:border-surface-200'
            }`}
          >
      ...
      <div className="space-y-2">
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-sm ${task.completed ? 'line-through text-surface-800/40' : 'text-surface-900'}`}>
                  {task.title}
                </p>
              ...
              {due && (
                <span className={`text-[10px] flex items-center gap-1 font-medium ${due.color}`}>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12">
            <CheckSquare size={32} className="text-surface-800/10 mx-auto mb-2" />
